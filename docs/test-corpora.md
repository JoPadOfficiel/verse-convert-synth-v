...
### Generated rhythm corpus

`src-tauri/tests/corpora/rhythm/` holds four MusicXML scores written by
[music21](https://github.com/cuthbertLab/music21) and committed. They carry no
third-party music: every note and syllable is generated by
`scripts/generate-rhythm-corpus.py`, so there is nothing to clear.

The generator runs out of CI and its output is the fixture, which keeps CI
offline and Python-free:

```sh
uv run --python 3.12 --with music21==10.5.0 python scripts/generate-rhythm-corpus.py
```

music21 is an independent producer here, not an oracle. Its exporter writes
shapes a hand-written fixture would not think to include — `<divisions>` stated
once per part and never repeated, `<voice>` omitted entirely for single-voice
music, an XML comment before every measure, content-hash part IDs, a DOCTYPE —
over rhythms chosen to be hard: 3-, 5-, 7- and 9-tuplets, a different tuplet and
meter in every bar, ties across every barline so no onset lands on one, and an
11-tuplet.

`defaults.divisionsPerQuarter` is 10080 = 2^5 x 3^2 x 5 x 7, so the first four
tuplets are exact and the 11-tuplet is not: music21 rounds each member and
overfills the bar by three divisions. Verse reads what it is given rather than
inventing the difference, and that file is in the corpus to keep it that way.

`src-tauri/tests/rhythm_corpus.rs` asserts the invariants over every file:
the stated `<divisions>` survives, no syllable is lost, no lane sounds two notes
at once, and each target accepts exactly the rhythms its own grid states —
computed from the grid, so adding a file needs no edit. Three of the four are
refused for OpenUtau, whose 480-tick quarter has no factor 7.
