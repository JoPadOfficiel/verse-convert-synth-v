    ...
    fn baseline_validator_rejects_restored_raw_final_tail_in_three_note_chain() {
        ...
        for target in [ExportTarget::Svp, ExportTarget::Ustx] {
            ...
            let head_origin = project.tracks[0].notes[0]
                .source_evidence.as_ref().unwrap().origin.as_ref().unwrap();
            let original_attack = raw.bindings[&(head_origin.track_id.clone(), head_origin.note_on_order)]
                .intensity.as_ref().unwrap().provenance.as_ref().unwrap();
            let original_ids: Vec<_> = original_attack.evidence.iter()
                .flat_map(|e| &e.source_ids).collect();
            assert!(!original_ids.is_empty());
            let transfers = verse_lib::engine::target::performance_report(target, &project).unwrap();
            for note in &project.tracks[0].notes[1..3] {
                assert_eq!(note.performance.as_ref().unwrap().intensity.as_ref().unwrap().provenance.as_ref(),
                    Some(original_attack), "raw source head, independent of continuation helper");
                let id = &note.source_evidence.as_ref().unwrap().note_id;
                if target == ExportTarget::Ustx {
                    assert!(transfers.iter().any(|span|
                        span.status == verse_lib::engine::performance::TransferStatus::Mapped
                        && span.note_ids.contains(id)
                        && original_ids.iter().all(|source_id| span.source_ids.contains(source_id))),
                        "emitted continuation must retain the original attack contributors: {id}");
                }
            }
            let mut missing_provenance = project.clone();
            std::sync::Arc::make_mut(missing_provenance.tracks[0].notes[2].performance.as_mut()
                .unwrap().intensity.as_mut().unwrap()).provenance = None;
            assert!(verse_lib::engine::target::serialize_to(target, &missing_provenance).is_err());
            assert!(verse_lib::bundle::BundleProject::from_projection(target, &missing_provenance).is_err());
            assert!(std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                validate_current_performance(&midi, &raw, &missing_provenance)
            })).is_err(), "independent baseline oracle must reject removed attack evidence");
            ...
            assert!(
                message.contains("current performance changed without exact source proof")
                    || message.contains("original tie attack provenance must be inherited unchanged"),
                "{message}"
            );
